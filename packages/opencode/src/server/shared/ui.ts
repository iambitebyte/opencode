...
export function embeddedUI(disableEmbeddedWebUi: boolean) {
  ...
  return (embeddedUIPromise ??= (async () => {
    // Compiled binary: bare specifier resolves to a Bun.build virtual module.
    try {
      // @ts-expect-error - generated file at build time
      const module = await import("opencode-web-ui.gen.ts")
      if (module.default) return module.default as Record<string, string>
    } catch {
      // fall through to on-disk manifest
    }
    // Source/dev mode: on-disk manifest generated by script/build-web.ts.
    try {
      const module = await import("./web-ui-manifest.gen")
      if (module.default) return module.default as unknown as Record<string, string>
    } catch {
